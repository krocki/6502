...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--pc', type=hex, help='program counter start')
    parser.add_argument('-f', '--framebuffer', type=hex, help='framebuffer offset')
    ...
    # sierpinski
    if opt.input:
      with open(opt.input, "rb"):
        prog = file.read()
    else:
    #simple sierpinski
      prog = [
       0xa9,0xe1,0x85,0x00,0xa9,0x01,0x85,0x01,0xa0,0x20,0xa2,0x00,0x41,0x00,0x91,0x00,
       0xe6,0x00,0xd0,0xf6,0xe6,0x01,0xa6,0x01,0xe0,0x06,0xd0,0xee,0x60,0x00,0x00,0x00
     ]

    frames = run_prg(prog,cpu_cycles,opt.output)
